# Public-release candidate review record — CRIT-APP

Reviewed: **30 July 2026**

Baseline reviewed against: `dbbe2cb97a0f4143847ce27e88509cbd724b6853`
...
- 28 curriculum chapters and 32 Markdown pages under `docs/`
- 89 current teaching figures
- 29 hash-locked transitive Python build dependencies
- 32 generated HTML pages, 11.48 MiB after post-processing
...
- Third-party fonts, icons, theme code, MathJax, trademarks, linked works, facts,
  and material outside the publisher's control are expressly excluded from the
  book-content grant and retain their own notices.
- The three self-hosted font families retain the SIL Open Font License 1.1,
  copyright notices, acquisition record, and exact hashes. Apache 2.0 text and
  applicable icon attribution are included in the release candidate.
- Every current figure has an exact hash and verified recorded-content Git
  ...
  asset register reports 19 conservative generator-source candidates, 70
  ...
  could be established were replaced with responsive semantic HTML/CSS; the
  absolute-effect calculation is also regression-tested. These records and
  replacements support traceability; they do not prove independent creation,
  copyrightability, identity, or ownership.
...
- No current-tree publisher PDF, article attachment, copied clinical
  instrument, office document, archive, or media file was found. No long
  block quotation was found in the curriculum.
...
## Clinical, research, and public-safety review

- Twenty-seven chapters received substantive final corrections; Chapter 17
  required no correction in the final pass.
- Corrections addressed randomization, intention-to-treat and per-protocol
  interpretation, target trials, DAGs, instrumental variables, confounding,
  OR/RR and hazard-ratio interpretation, NNT, p-values, missing data,
  multiplicity, subgroups, diagnostic reference standards, prevalence,
  calibration, decision-curve analysis, external validation, and AI leakage.
- Universal clinical commands and categorical claims that methods, metrics,
  checklists, or validation prove safety or validity were removed. Vignettes
  are educational and governance-bound; group-average effects are not presented
  as proof of individual benefit.
...
- The evidence register is explicitly bounded to selected consequential claims
  ...
  peer review.
- Quality-improvement and case-review exercises do not authorize collection of
  patient information or replace local privacy, security, IRB, or other
...
- The release scanner checks the complete tracked current tree and the rendered
  ...
  module suffixes and the configured `/CRIT-APP/` 404-page path form, must
  resolve to an exact allowlisted path and digest; remote script markup is
  rejected.
...
- GitHub Actions are commit-SHA pinned. Pull requests run all release gates;
  ...
  Checkout transfers full event-reachable history because provenance checks
  and release/current-pull-request secret scanning require historical object
  contents.
...
- rendered post-processing: 32 HTML files, 94 enhanced images, 94 semantic
  figures, 64 accessible toggle controls
...
Before this hardening commit, 1,050 commits were reachable across fetched
branch, tag, and public pull-request refs. A post-commit Gitleaks 8.30.1 scan
including the candidate covered all 1,051 reachable commits as text (about
1.64 GB) and found no leak. The pre-hardening all-object identifier/path/type
screen covered 39,794 objects and 33,436 blobs and found no SSN, MRN, DOB,
private-key marker, or suspicious document/archive/database extension in the
tested patterns.
...
The all-ref image-object screen found 27,254 historical image paths and 27,255
unique reachable image blobs (27,252 PNG and 3 SVG). Eighty-nine image blobs are
in the current release, leaving 27,166 non-current blobs reachable through
public history. Of the PNG blobs, 27,249 carry Matplotlib software metadata;
three carry no detected creator marker. One additional historical PNG,
`docs/assets/figures/cycle1468_swarm_ch02_w1468_2.png`, is truncated and cannot
be fully decoded. No EXIF/IPTC/XMP identity-or-location field,
Adobe/other creator marker, structured copyright/trademark/attribution marker,
or active/remote SVG feature was found by the tested metadata and text
patterns. This automated screen is not raster OCR, visual similarity review,
source-data review, chain-of-title proof, or legal clearance of every
superseded image. History was not rewritten.
