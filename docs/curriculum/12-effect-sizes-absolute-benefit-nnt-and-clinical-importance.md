...
Odds Ratio (OR): Odds are p / (1 - p), and the OR compares treatment and control odds. When outcome probabilities are low in both groups, the OR may approximate the RR; as outcomes become more common, the measures can diverge. The OR is not inflated or invalid on its own scale, but interpreting it as a relative change in probability overstates that probability contrast when it is farther from 1 than the RR.
...
When an ARR interval crosses zero (e.g., ARR 0.02, 95% CI −0.01 to 0.05), the reciprocal scale is discontinuous: the limits correspond to NNT 20 on the benefit side and NNH 100 on the harm side, with unbounded values near zero. Report the risk-difference interval directly; an NNT point estimate alone would conceal compatible no-effect and harm values.
...
For a matching two-sided test and confidence level under the same model, statistical significance corresponds to a confidence interval that excludes the null. This does not establish clinical importance. The examples below distinguish five interval patterns relative to a prespecified minimal important difference (MID):
...
The p-value depends on effect magnitude, sampling variability, model, and sample size. A small effect can produce a small p-value in a sufficiently precise study, while a large but imprecise estimate may not. The numerical p-values cannot be inferred from sample size and point estimate alone; inspect the effect estimate and interval rather than using these examples as guaranteed outcomes.
...
Failure Mode 2: Misinterpreting Odds Ratios as Risk Ratios. For common outcomes, the OR is usually farther from 1 than the corresponding RR, but it is not an exaggerated estimate when interpreted on the odds scale. Verify the metric. Use raw counts to calculate a crude absolute difference when available, and obtain model-based standardized risks when the target estimate is adjusted or involves time-to-event, clustering, or covariate-dependent effects.

Failure Mode 3: Prediction is not Causation. Inverting a crude treated-versus-untreated risk difference from a registry does not create a causal NNT. An observational estimate can support a causal interpretation only for a well-defined intervention under defensible identification assumptions, design, measurement, positivity, and analysis. Methods such as weighting, valid instruments, or target-trial emulation do not certify that those assumptions hold; state them and quantify sensitivity.

Failure Mode 4: Ignorance of Time Horizons. A Cox model estimates a hazard ratio under its model; with nonproportional hazards, a fitted constant coefficient can be a difficult-to-interpret weighted summary. A fixed-horizon risk difference has a different estimand and should be estimated with methods appropriate to censoring and competing events. Do not extrapolate NNT linearly across time because event hazards and treatment effects can vary.
...
Note 2: Interaction on Additive versus Multiplicative Scales. The default scale depends on the fitted model: logistic and Cox product terms assess interaction on odds- or hazard-ratio scales, while identity-link models can target additive interaction. Two subgroups can have similar relative effects but different absolute effects when baseline risks differ. Report clinically relevant subgroup-specific absolute risks or differences with uncertainty as well as the prespecified interaction contrast; no single scale is universally decisive.
