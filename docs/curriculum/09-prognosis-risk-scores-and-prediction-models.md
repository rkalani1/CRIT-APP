# Chapter 9. Prognosis, Risk Scores, and Prediction Models

## Opening

![ROC versus calibration.](../assets/figures/fig29_roc_vs_calibration.png)

*Discrimination (the ROC curve) and calibration answer different questions; a model can rank patients well yet output probabilities that are systematically wrong.*

![Calibration plot concept.](../assets/figures/fig07_calibration.png)

*A calibration plot compares predicted risk to observed frequency; points off the diagonal reveal over- or under-prediction.*

![Calibration of predicted versus observed risk.](../assets/figures/swarm_ch09_calibration.png)

*Calibration must hold in patients like yours, not just in the derivation sample, before a score guides discharge planning.*

A prognostic score promises discharge planning precision. Demand calibration in patients like yours, not only a shiny C-statistic from the derivation sample.


## The Demarcation of Prediction from Causal Inference

Clinical neurology generates scoring systems at a relentless pace. The National Institutes of Health Stroke Scale (NIHSS) originated as a reproducible severity measure but rapidly mutated into a prognostic heuristic. The ICH Score predicts thirty-day mortality after intracerebral hemorrhage. The ABCD2 score stratifies the short-term probability of stroke following a transient ischemic attack. Modern machine learning algorithms digest raw non-contrast computed tomography images to predict large vessel occlusion, while others aggregate electronic health record data to forecast malignant cerebral edema. Despite this saturation, the fundamental error committed by clinicians and researchers alike is conflating a prediction model with a causal model. This conflation routinely poisons journal clubs and distorts clinical guidelines.

A prediction paper estimates a future outcome from inputs available at a stated prediction time and evaluates forecast performance. A causal treatment paper estimates how assigning one intervention rather than another changes an outcome under explicit assumptions. A prediction study may discuss causal hypotheses, but predictive performance alone does not establish them; keep the two target questions separate.

If an observational stroke registry derives a multivariable logistic regression model for ninety-day functional independence and reports that statin use at admission carries an odds ratio of 1.4, this constitutes a predictive association. It indicates that statin users, within the specific context and confounding structure of that dataset, possess a higher probability of a favorable outcome. It explicitly does not mean that initiating a statin in the emergency department causes the good outcome. The statin variable acts as a proxy; it likely identifies patients who have consistent primary care access, fewer swallowing deficits, or lower baseline frailty. When audiences pivot seamlessly from an impressive area under the receiver operating characteristic curve (AUROC) to concluding 'we should treat these patients differently based on these variables,' they commit a severe category error. Prediction tools rank patients by risk; they do not identify which patients benefit from a specific intervention. Identifying benefit requires treatment effect heterogeneity analysis within a causal framework.

Rigorous appraisal of prognosis and prediction literature treats discrimination (AUROC) as only one dimension of performance. Calibration, clinical utility, strict time-zero discipline, control of overfitting, and appropriate validation structures determine whether predictions are trustworthy and useful. TRIPOD supports transparent reporting, while PROBAST structures risk-of-bias assessment.

## Taxonomy of Claims: Prognosis, Prediction, and Diagnosis

Scientific claims in the medical literature divide into diagnostic, prognostic, and causal categories. Diagnosis asks about a target condition at a clinically relevant time, using a reference standard that may be applied shortly before or after the index test (see Chapter 8). Its major threats include verification bias, spectrum effects, and imperfect reference standards. Prognosis instead predicts a future outcome over a defined horizon.

Prognosis asks: What will happen to this patient in the future under a defined, existing clinical care pathway? What is the probability of symptomatic intracranial hemorrhage after the administration of intravenous alteplase? What is the chance of regaining independent ambulation at six months post-stroke? Prognosis inherently involves the forward passage of time. Consequently, its primary validity threats involve longitudinal mechanics: loss to follow-up (informative censoring), competing risks (such as death occurring prior to the planned functional assessment), and secular changes in the standard of care over the follow-up period.

Prediction refers to the statistical machinery utilized to estimate individualized probabilities. Modern literature uses the term 'prediction model' loosely to encompass both diagnostic classifiers and prognostic risk scores. This conflation is problematic because time enforces a strict logic on prognostic prediction that diagnostic modeling lacks. A diagnostic algorithm for large vessel occlusion on CT angiography evaluates synchronous pixels. A prognostic algorithm for ninety-day modified Rankin Scale (mRS) must strictly segregate data available at the exact moment of prediction from events that unfold subsequently. Your first task as an appraiser is forcing clarity: Is the model estimating a concurrent state or forecasting a future event?

Crucially, prognosis is not equivalent to the natural history of a disease under a state of therapeutic nihilism. Modern neurological care is intensely interventional. Prognostic models estimated in the era of mechanical thrombectomy describe outcomes under that specific prevailing care regime. They are valid for counseling only if the patient receives that standard of care. If authors imply their prognostic model identifies patients who 'need' thrombectomy without formally estimating the interaction between the baseline features and the treatment effect (a causal claim), they have overstepped the mathematical boundaries of their design.

## Anatomy of a Prediction Model: Time Zero, Predictors, and Horizon

The structural integrity of a prediction model rests on three pillars: time zero, the predictor set, and the prediction horizon. Time zero (or index time) is the precise, definable moment in the clinical workflow when the model is intended to be executed. A model's validity hinges entirely on uncompromising adherence to time zero discipline. If a tool is designed to guide emergency department triage prior to advanced neuroimaging, its time zero is the moment of emergency department arrival. At that precise second, age, vital signs, and baseline NIHSS are knowable. Final infarct volume, door-to-needle time, and hospital-acquired complications are not.

Index-time leakage constitutes a fatal flaw in predictive modeling. Leakage occurs when information from the future—or information generated as a consequence of the outcome pathway—surreptitiously bleeds into the predictor set. A classic failure mode in the stroke literature involves predicting ninety-day functional outcome using 'final infarct volume on day 3 MRI' or 'discharge disposition' as baseline features for an admission model. These variables correlate massively with the outcome precisely because they are intermediate steps on the causal pathway to that outcome. Incorporating them inflates apparent accuracy to near-perfect levels, but renders the model clinically useless. A clinician cannot input a discharge disposition on day one.

Electronic health record models are uniquely vulnerable to stealth leakage. A billing code for 'cerebral edema' might be timestamped at hospital day three, but if the researcher crudely pulls all codes associated with the encounter into a 'baseline' matrix, the future has leaked into the past. Clinical notes utilized for natural language processing routinely contain trailing information; an admission note dictated twelve hours after arrival may casually reference the patient's subsequent deterioration or response to initial therapy. Timestamp discipline must be absolute. The appraiser must ask: Could a conscientious clinician, standing at time zero, execute this equation using only data mathematically confirmed to exist at that moment?

The prediction horizon should align with the intended decision, and predictors should be reproducibly defined and available at the intended prediction time. Arbitrary dichotomization of a continuous predictor usually loses information and imposes a discontinuous step relation. Flexible functions such as restricted cubic splines or fractional polynomials are common options, but linear terms, monotonic models, prespecified clinical categories, or other forms may be appropriate depending on the estimand, data, sample size, and intended implementation. Validate the functional form rather than mandating one family.

## Quantitative Reasoning: Discrimination

Discrimination quantifies a prediction model's ability to separate patients who experience the outcome from those who do not. It asks a strictly relative question: Does the algorithm consistently assign higher predicted probabilities to the cohort of patients who actually suffer the event compared to those who remain event-free?

For binary outcomes, discrimination is commonly summarized by the c-statistic, equivalent to AUROC. Consider all usable event/non-event pairs. A pair is concordant when the model assigns higher risk to the participant with the event and tied when scores match. The formula is c = (concordant + 0.5 × tied) / (concordant + discordant + tied).

An AUROC of 0.50 indicates chance-level ranking and 1.0 indicates perfect separation in the evaluated data. An AUROC of 0.75 means that, for a randomly selected event/non-event pair, the model gives the event case a higher score with probability 0.75 when there are no ties; the conventional definition assigns half credit to tied scores. For censored time-to-event outcomes, concordance measures such as Harrell's C use comparable pairs and require their own censoring assumptions.

AUROC is often overinterpreted as clinical value. It measures rank-order separation, not agreement between predicted probabilities and observed frequencies. A monotonic transformation of scores can preserve AUROC while radically changing probability calibration. A model that ranks perfectly but assigns implausible absolute risks may be unusable for probability counseling or threshold decisions even though its AUROC is 1.0 in the evaluated data.

With rare outcomes, AUROC can coexist with poor positive predictive value at deployment prevalence. Report precision-recall performance, sensitivity and specificity, PPV and NPV at clinically relevant thresholds, and calibration; no single curve is universally superior. AUROC summarizes ranking between event and non-event cases, while precision-recall performance is prevalence-sensitive and often makes false-alarm burden more visible.

## Quantitative Reasoning: Calibration

Calibration measures the degree of agreement between estimated probabilities and the actual observed frequencies of the outcome. It asks the definitive absolute question: If the model dictates that a patient has a 20% risk of symptomatic intracerebral hemorrhage, and we assemble 100 identical patients, do approximately 20 of them actually bleed?

Calibration is essential when predicted probabilities guide decisions, resource allocation, or counseling, but it is not sufficient: discrimination, clinical utility, uncertainty, and transportability also matter. A high-AUROC model can still provide badly calibrated absolute risks.

Calibration-in-the-large compares the overall mean predicted probability to the overall observed event rate in the entire dataset. If the model predicts an average cohort risk of 15%, but the actual observed event rate is 30%, the model is systematically underpredicting. The simplest summary of calibration-in-the-large is the Observed/Expected (O/E) ratio: Total Observed Events divided by Total Expected Events (the sum of all predicted probabilities). The ideal O/E ratio is exactly 1.0.

Calibration curves compare predicted probabilities with observed frequencies across the risk range; flexible smoothers are generally more informative than a small number of bins. Calibration-in-the-large is estimated by recalibrating the intercept with the original linear predictor's coefficient fixed at 1. A calibration model that jointly estimates intercept and slope describes both overall level and spread, so its intercept should be interpreted with the slope. Ideal values are 0 and 1. In validation data, a slope below 1 indicates predictions that are too extreme in that setting; development overfitting is one cause, while case-mix differences, coefficient drift, and measurement changes can also contribute. A slope above 1 indicates predictions that are not extreme enough in that setting.

The Brier Score provides an omnibus measure of predictive accuracy, inextricably blending both discrimination and calibration. It is calculated as the mean squared difference between the predicted probability and the actual outcome (coded strictly as 1 for an event and 0 for no event). The formula is: Brier Score = (1/N) * sum(predicted_i - observed_i)^2. For a model predicting an event with a 50% baseline prevalence, blind random guessing yields a Brier score of 0.25. Lower Brier scores indicate superior overall performance, but a low Brier score does not absolve the researcher from presenting a detailed calibration plot.

## Development, Overfitting, and Optimism

When effective model complexity is high relative to outcome information, apparent performance is prone to optimism and selected patterns may reflect sample-specific noise. The risk depends on candidate parameters, outcome frequency, modeling choices, penalization, and whether the entire development process is internally validated.

This phenomenon is overfitting. Apparent performance (the AUROC and calibration metrics calculated on the exact same dataset utilized to train the model) is typically optimistic. Optimism is the difference between apparent performance and expected performance in new, exchangeable data. A low ratio of outcome events to model complexity increases risk; simple events-per-variable rules are incomplete because complexity also depends on parameterization, transformations, interactions, penalization, and the selection process. Screening many candidates and selecting on the same data adds selection-induced optimism, even when only a few variables appear in the final equation.

Internal validation quantifies optimism without requiring a separate external dataset. Common approaches include resampling-based cross-validation and bootstrapping; a single split can be unstable and inefficient in small datasets. In bootstrap optimism correction, draw bootstrap samples, rerun the entire modeling pipeline in each sample, compare performance in that bootstrap sample with performance when the fitted pipeline is applied to the original data, average the optimism, and subtract it from apparent performance. The method, resampling structure, and full pipeline must be reported.

Internal validation intended to estimate the performance of a development procedure must repeat every data-driven step, including screening, transformations, tuning, and selection. Validating only a fixed final equation can describe that equation's resampling behavior in the available data, but it underestimates optimism from the omitted selection process and does not validate the full pipeline.

## External Validation and Transportability

Internal validation estimates optimism under the development process; it does not prove correctness or transportability. External validation applies the original model—including its intercept and coefficients—to new data from a relevant place, time, or population and evaluates calibration and discrimination. Re-estimating coefficients is model updating and should be distinguished from the initial validation analysis.

Prediction models fail external validation for three primary reasons. First, True Overfitting: The original model captured statistical noise rather than biological signal, which obviously fails to replicate in new patients. Second, Case-mix variation: The external cohort possesses a different distribution of predictor variables or a different baseline risk. If a prediction tool for malignant middle cerebral artery syndrome is developed in a tertiary referral center (which receives a high prevalence of massive, catastrophic strokes) and tested in a community hospital (which sees a lower prevalence), the discrimination might remain stable, but the calibration will systematically drift, rendering absolute risk predictions inaccurate.

Third, Temporal drift: Medical care is not static. The ASTRAL score was developed in the early 2010s to predict 90-day outcomes after acute ischemic stroke. Treatment-era changes can alter calibration in contemporary thrombectomy-treated populations; verify this empirically and recalibrate or update only when validation demonstrates drift. A legacy score should not be assumed current without validation in the intended setting.

## Decision Curve Analysis and Clinical Utility

Calibration assesses agreement between predicted and observed risks. Decision-curve analysis evaluates a model-guided strategy across specified threshold probabilities. AUROC alone cannot establish clinical utility: deployment also depends on calibration, thresholds, alternatives, consequences of false decisions, workflow, and uncertainty.

DCA requires the explicit definition of a Threshold Probability (Pt). Pt is the specific risk level at which a clinician is perfectly indifferent between taking the action and withholding it. If you decide to order a high-risk CT angiogram for a patient with a transient ischemic attack only if the probability of an underlying severe stenosis exceeds 10%, your Pt is 10%. This threshold inherently encodes the clinician's judgment regarding the relative harm of a false positive versus a false negative. Specifically, the harm ratio is calculated as Pt / (1 - Pt). At a 10% threshold, you are mathematically stating that missing a true positive is 9 times worse than enduring a false positive (0.10 / 0.90 = 1/9).

Decision Curve Analysis calculates the Net Benefit of using the prediction model across a continuous range of plausible threshold probabilities. The formula for Net Benefit is: Net Benefit = (True Positives / N) - (False Positives / N) * (Pt / (1 - Pt)). The subtraction of the weighted false positives ensures that the model is penalized according to the specific clinical harms defined by the threshold.

In a standard DCA plot, the Net Benefit of the prediction model (y-axis) is graphed against the Threshold Probability (x-axis). Crucially, the model is compared simultaneously against two default clinical strategies: 1) Treat All (acting as if everyone has the outcome). The Net Benefit line for 'Treat All' slopes downward as the threshold increases, crossing zero exactly at the overall prevalence of the disease in the cohort. 2) Treat None (acting as if nobody has the outcome). The Net Benefit for 'Treat None' is exactly zero across all thresholds.

![Schematic decision curve comparing model-guided, treat-all, and treat-none strategies across threshold probabilities.](../assets/figures/fig52_decision_curve.png)

*Schematic only—not fitted study data. A model has net benefit only over threshold ranges where its curve lies above the relevant alternatives.*

Decision-curve analysis compares modeled strategies under an explicit weighting of false positives and false negatives encoded by the threshold probability. Net benefit above relevant default strategies at plausible thresholds is supportive, but not sufficient, evidence of utility; the calculation also depends on calibration, the modeled action, and assumptions about consequences. If positive net benefit appears only at thresholds far from the intended decision range, the analysis does not support that intended use. Prospective impact, workflow, equity, and safety still require evaluation.

## Named Frameworks: TRIPOD and PROBAST

TRIPOD provides reporting guidance for studies that develop, validate, or update multivariable prediction models. It asks authors to describe setting, participants, predictors, outcomes, sample-size reasoning, missing-data methods, model specification, and performance. Complete reporting enables appraisal but does not itself establish low risk of bias; complete-case analysis and multiple imputation each require assumptions and can each be inappropriate if poorly specified.

PROBAST (Prediction model Risk Of Bias Assessment Tool) is the corresponding appraisal instrument used to evaluate the methodological quality of the published model. PROBAST forces the appraiser to evaluate Risk of Bias systematically across four critical domains: Participants (detecting inappropriate exclusions or highly selected cohorts), Predictors (identifying predictors defined using knowledge from the future), Outcome (flagging subjective outcomes adjudicated with unblinded knowledge of the predictors), and Analysis.

PROBAST's Analysis domain examines participant counts and event information, predictor handling, missing data, competing risks, overfitting, performance evaluation, and optimism. Discarding 40% of a cohort through complete-case analysis raises major concerns about efficiency, target population, and selection; bias direction depends on the missingness mechanism and analysis model. Automated selection among 100 variables with only 50 events and no internal validation is highly prone to overfitting, so an apparent AUROC of 0.92 is not a credible generalization estimate. PROBAST structures a domain judgment rather than supplying an automatic paper-level verdict.

## A Fully Worked Example: Evaluating a Post-Thrombectomy Prognostic Score

Scenario: A recent publication proposes the 'REPERFUSE-3' score to predict 90-day catastrophic outcome (defined as mRS 5-6) following mechanical thrombectomy. The authors develop the score on a cohort of 1,000 patients, observing 270 events (27% baseline prevalence). They assign points as follows: Age > 75 (1 point), Admission NIHSS > 15 (1 point), and Core Volume > 50cc (1 point). The maximum score is 3. They report an AUROC of 0.75 and conclude the model 'can guide aggressive end-of-life decision making in the neuro-ICU.' They provide the following raw validation data: Score 0 (N=300, Predicted Risk 0.05, Observed Events 15); Score 1 (N=400, Predicted Risk 0.15, Observed Events 80); Score 2 (N=200, Predicted Risk 0.40, Observed Events 90); Score 3 (N=100, Predicted Risk 0.85, Observed Events 85).

Step 1: Appraisal of the Claim and Time Zero. The proposed clinical action is 'end-of-life decision making.' The predictors (age, NIHSS, core volume) are available at admission, prior to thrombectomy. However, the score is named 'REPERFUSE' and the cohort consists exclusively of post-thrombectomy patients. Did the modeling process include final reperfusion status (e.g., TICI score)? If so, time zero is post-procedure, and the tool cannot be used for pre-procedure counseling. If not, time zero is pre-procedure, but the model is conditioned on the patient actually undergoing the procedure.

Step 2: Calibration Check. Expected events are 300×0.05 + 400×0.15 + 200×0.40 + 100×0.85 = 240, while 270 are observed, so O/E = 270/240 = 1.125: the model underpredicts overall. Score 0 and Score 3 match their grouped predictions, but Score 1 is 20% observed versus 15% predicted and Score 2 is 45% versus 40%. Checking only the extremes would miss this intermediate underprediction. Request a calibration intercept and slope, a smoothed calibration curve, and uncertainty rather than declaring the absolute risks trustworthy.

Step 3: Calculating Discrimination Intuition. The absolute risk difference between a maximum score of 3 (85% observed risk) and a minimum score of 0 (5% observed risk) is 80%. The model clearly separates the highest and lowest risk groups effectively, which mathematically aligns with the reported AUROC of 0.75. However, separating extremes is easy; the clinical challenge lies in managing the intermediate groups.

Step 4: Synthetic Net-Benefit Illustration. Replace the unsafe action of treatment withdrawal with a non-life-limiting action: trigger multidisciplinary prognosis review and a structured goals-of-care discussion while continuing indicated treatment. For teaching arithmetic only, suppose a 50% threshold encodes equal weight for an unnecessary review and a missed review opportunity. The model triggers review only for Score 3.

Under this synthetic utility assumption, TP = 85, FP = 15, N = 1,000, and net benefit = 85/1,000 − (15/1,000)×1 = 0.070.

Step 5: Compare default strategies under the same assumed utility. Review-all gives 270/1,000 − (730/1,000)×1 = −0.460; review-none is 0; model-guided review is 0.070. This illustrates threshold-dependent decision-curve arithmetic, not proven clinical utility. Decision-curve analysis cannot validate treatment withdrawal or establish that acting on the prognostic classification improves outcomes.

Step 6: Sensitivity to Thresholds. At a synthetic 90% threshold, no score category triggers review and model net benefit is 0, matching review-none. The exercise shows why discrimination alone cannot determine a decision threshold; utilities, calibration, and the intended action must be specified and tested prospectively.

## Pitfalls and Failure Modes in Neurologic Prediction

- Unnecessary dichotomization: Collapsing a continuous variable—such as systolic blood pressure, core volume, or age—into a binary category discards within-category information and imposes an abrupt step at the cutoff. That can reduce precision, obscure nonlinear relationships, and harm calibration. A threshold may still be justified by an actual decision rule, measurement limit, or prespecified clinical use, but the model should not invent one merely for convenience.
- The Immortal Time Fallacy in Prediction: Defining a study cohort strictly as 'patients who underwent a 90-day follow-up MRI.' This inclusion criterion guarantees that any patient who died prior to day 90 is systematically excluded from the dataset. If the researcher subsequently builds a model predicting 90-day functional independence, the resulting algorithm is entirely conditional on survival. It cannot be applied to a newly admitted patient in the emergency department, because the clinician at time zero does not possess the future knowledge of whether the patient will survive to day 90.
- Competing risks ignored: For a five-year recurrent-stroke prediction, death can preclude the event. Treating competing deaths as ordinary censoring and reporting one minus Kaplan-Meier survival generally overestimates cumulative incidence under the usual assumptions. Use a cumulative-incidence estimand and a model aligned with the prediction question, while reporting how competing events were defined (see Chapter 11).
- Uncalibrated machine learning: A complex model can rank patients well while its predicted probabilities disagree with observed risks. Inspect calibration-in-the-large, slope, and plots in relevant validation data. Recalibration methods such as logistic recalibration or isotonic regression may help when justified, but they must be estimated without contaminating final testing and do not repair an invalid cohort, label, or time zero.

## Clinical and Epidemiologic Notes

- Clinical Note — The CHA₂DS₂-VASc vs HAS-BLED Illusion: CHA₂DS₂-VASc estimates stroke risk and HAS-BLED estimates bleeding risk in atrial fibrillation, but overlapping prognostic factors can raise both. Neither score alone identifies an individual causal treatment effect. Using a high prognostic score as proof of a larger relative benefit from anticoagulation is a separate claim that requires treatment-effect evidence; the scores primarily characterize baseline risk under their target settings.
- Clinical Note — ABCD2 and Case-Mix Drift: ABCD2 performance has varied substantially across cohorts. The original derivation and validation work reported c-statistics ranging roughly from 0.60 to 0.81, while later emergency-department studies and systematic reviews found only modest discrimination in many settings. Differences in TIA adjudication, mimic prevalence, treatment, and case mix can change both discrimination and calibration. A score intended for undifferentiated emergency patients should therefore be validated in that workflow and should not replace etiologic evaluation or expert assessment.
- Clinical Note — Algorithmic Nihilism and the ICH Score: The ICH Score predicts 30-day mortality, but it should not be used alone to justify early withdrawal of life-sustaining therapy. Early treatment limitation can contribute to a self-fulfilling association between score and mortality; prognostic studies should report treatment limitations and avoid treating the score as a causal verdict about an individual patient.
- Research Design Note — Absolute Effects over Relative Risks: Individual predictor odds ratios or hazard ratios do not by themselves tell a patient’s absolute prognosis or the benefit of acting on a model. Demand calibrated absolute probabilities at a named horizon. If a causal fixed-horizon risk ratio is credible and transportable, then ARR = control risk × (1 − RR), and NNT is the reciprocal of that ARR. This shortcut does not apply directly to an odds ratio, a hazard ratio, or an arbitrary prognostic risk without the required conversion and causal assumptions.


## Chapter summary

Prognosis and prediction models forecast outcomes from information available at a defined time zero; they do not by themselves identify treatment effects. Appraisal separates discrimination from calibration and checks whether the development process, validation, and handling of missing data estimate performance without future-information leakage. Internal validation can estimate optimism, and evaluation in data separated by place or time is often needed before transport claims are credible. Decision-curve analysis examines model-based decisions under explicit threshold and utility assumptions; it is not an impact study. PROBAST structures risk-of-bias and applicability judgments but does not certify a model for bedside use.

## Practice and reflection

1. Select a recent stroke risk score publication. Write a single sentence strictly defining the intended population, the precise time zero, the predictor set, the outcome, and the prediction horizon. Flag any predictor that violates time zero discipline.
2. Explain to a junior resident, utilizing plain language, how a prediction model can achieve an AUROC of 0.85 yet be actively dangerous for counseling patients regarding absolute risk.
3. Using a hypothetical dataset of 1000 patients with a 20% baseline prevalence of poor outcome, calculate the Net Benefit of a 'Treat All' strategy at a threshold probability of 30%.
4. Outline a comprehensive external validation protocol for an admission ICH expansion model, explicitly addressing the threats of geographical case-mix variation and temporal changes in blood pressure management.
5. Identify a published neurology paper that relies entirely on split-sample internal validation for a multivariable model. Describe how executing bootstrap validation on the full pipeline would likely alter their reported performance.
6. Review the PROBAST criteria. For a complete-case analysis that discards 20% of a cohort because a continuous predictor is missing, state the assumptions under which the estimate could remain unbiased, the mechanisms that could introduce bias or change the target population, and the likely loss of precision.
7. Contrast the clinical utility of the ICH Score when used to stratify patients for a clinical trial versus when used to guide the withdrawal of life-sustaining therapy on hospital day one.
8. A paper reports an adjusted odds ratio of 3.5 for a novel biomarker predicting 90-day stroke recurrence and a marginal baseline risk of 2%. State why those quantities are insufficient to calculate an individual probability without the model intercept, covariate values, and reference definition. Then perform the conversion only under an explicitly simple unadjusted two-group model with a 2% reference-group risk, and state the additional causal assumptions needed before calculating an NNT for an intervention.

---

*Figures and tables in this chapter are Teaching materials for CRIT-APP unless a caption explicitly states otherwise. Methods standards are cited by name only.*
