...
Consider the ABCD2 score for TIA risk stratification. Its discrimination, calibration, and ability to identify every high-risk mechanism vary across settings; a low score does not by itself exclude carotid stenosis, atrial fibrillation, or another urgent cause. Appraise the intended use and current guideline rather than using a risk score as a substitute for diagnostic evaluation. Even a well-validated prediction rule has limited value if it does not improve the defined decision compared with current practice or if its failure modes create net harm.

## Appraising the Intersection: When Systematic Reviews Evaluate CPRs

Systematic reviews of prediction models often identify many competing scores but fewer independent validations and impact evaluations. The balance varies by clinical domain and publication date, so extract model-specific evidence rather than assuming every review reaches the same conclusion. Common concerns include small effective sample size, missing-data handling, overfitting, selective reporting, calibration, and applicability.

The takeaway is rarely to adopt a score merely because it ranks highest in a review. Determine whether the intended setting needs a new derivation study or, more often, independent head-to-head validation, recalibration, and impact evaluation of existing models. Use a rule only for its validated intended role, alongside required clinical evaluation and current guidance, and monitor whether its absolute risks and failure modes remain appropriate in the current setting.
