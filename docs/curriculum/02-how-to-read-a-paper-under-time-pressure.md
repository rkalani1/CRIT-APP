...
This chapter assumes a foundational commitment to distinguishing prediction from causation, and absolute effects from relative effects. Those distinctions become operational in the paper's methods, results, and data displays, where they can be extracted before cognitive fatigue sets in. Intentional, recorded skipping is appropriate when it remains aligned with the clinical decision at hand. Every minute spent on unconstrained extrapolation in the discussion is a minute unavailable for verifying the index time definition or missing-data handling in the methods; the latter takes priority.
