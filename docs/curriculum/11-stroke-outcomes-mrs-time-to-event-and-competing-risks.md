...
Trial conclusions depend on how outcomes, horizons, and summary measures are defined. A result may cross a significance threshold for mRS 0-2 versus 3-6 but not for mRS 0-1 versus 2-6; that does not make one threshold intrinsically correct or the trial a success or failure. Similarly, censoring competing death and reporting 1 minus Kaplan–Meier targets a different quantity from real-world cumulative incidence. Outcome choice therefore combines a values judgment with a methodologic design choice, and endpoint literacy is necessary to determine what question the analysis answers.

Prediction is not causation. Predicting 90-day mRS from baseline NIHSS estimates an outcome distribution from observed information. Estimating the effect of assigning tenecteplase rather than a comparator requires a counterfactual contrast under explicit assumptions. Some baseline predictors, such as age, are not directly assignable interventions; others may be related to modifiable processes but still cannot be interpreted causally from a prediction coefficient. Confusing the frameworks can lead to serious errors such as conditioning on post-randomization variables or treating an associational hazard ratio as a causal effect.
...
Including death as mRS 6 makes the scale a joint ordering of survival and disability. A therapy that prevents death but leaves additional survivors at mRS 5 shifts some outcomes from 6 to 5 and is counted as improvement by an ordinal analysis. Individuals may value those states differently, so report mortality and the full category distribution alongside a common-odds summary rather than assuming that the model's ordering resolves patient preferences.

mRS ascertainment is vulnerable to inter-rater variability and knowledge of treatment, especially near category boundaries. Structured interviews, assessor training, blinding where feasible, central review, and reliability monitoring can reduce this risk, but no single instrument or remote workflow eliminates it. Appraise the method used, any evidence of unmasking, and the likely sensitivity of the endpoint to judgment.
...
For common outcomes, an odds ratio is farther from 1 than the corresponding risk ratio and should not be described as a relative change in probability. If mRS 0-2 occurs in 30% versus 40%, the risk difference is 10 points, the risk ratio is 1.33, and the odds ratio is 1.55. Each is valid for its stated scale; calling the OR a '55 percent improvement in probability' is incorrect. Report group-specific risks and differences with uncertainty alongside relative measures.

Outcome-threshold switching threatens confirmatory interpretation and type I error control. If investigators prespecify mRS 0-1, inspect the data, and then highlight mRS 0-2, the latter result is post hoc unless a valid prespecified multiplicity plan says otherwise. Compare the publication with the dated protocol, statistical analysis plan, registry, and amendments; a discrepancy does not invalidate every descriptive result, but it changes which claims are confirmatory.
...
The proportional-odds assumption can fail when effects differ across cutpoints. As a synthetic illustration, a craniectomy strategy might primarily shift outcomes from death to severe disability while producing little change at mild-disability cutpoints. A common odds ratio can then obscure clinically important nonuniformity. Inspect the full category distribution, assess model fit, and consider prespecified partial proportional-odds, sliding-dichotomy, utility, or other estimand-aligned analyses rather than declaring the summary universally uninterpretable.
...
Clinical Note: Bedside Communication. Do not present a hazard ratio or relative risk without compatible absolute probabilities at a meaningful horizon. A 50% relative reduction from 2% to 1% is a 1-point absolute difference; whether that is negligible depends on outcome importance, harms, burden, uncertainty, and patient values. A calibrated statement is: 'In the study, about 10 in 100 experienced the outcome by one year with the comparator and 6 in 100 with treatment—about 4 fewer per 100 on average, if the estimate applies here.' Avoid implying which individual benefits.
...
Epidemiologic Note: Missing Data Mechanisms. Under MCAR, complete-case estimates can be unbiased for some estimands but less precise. Under MAR conditional on variables in a correctly specified analysis or imputation model, methods such as multiple imputation can be valid. MNAR means missingness still depends on unobserved values after conditioning; it is not identified from observed data alone. Selection models, pattern-mixture models, bounds, and tipping-point analyses encode different unverifiable assumptions. Report reasons and timing, include predictors of missingness and outcome when justified, and test conclusions across plausible MNAR scenarios.
