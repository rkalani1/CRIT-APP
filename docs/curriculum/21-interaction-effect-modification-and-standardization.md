...
The stroke literature frequently confuses confounding, interaction, and effect modification. Confounder control targets identification of a chosen marginal or conditional effect; effect heterogeneity is assessed on a named scale with interaction contrasts, stratified estimates, standardization, or flexible models. A variable can be both a confounder and an effect modifier, so these roles are not mutually exclusive labels. If a device effect varies by clot composition, report the composition-specific estimates and uncertainty on clinically relevant scales rather than assuming a uniform average is sufficient.
...
Logistic and Cox product terms operate on odds- and hazard-ratio scales unless transformed. A nonsignificant interaction p-value on one scale does not establish homogeneity on another. Additive effects often help quantify population impact and clinical tradeoffs, but decisions also require outcome importance, uncertainty, harms, feasibility, and preferences rather than a risk difference alone.
...
A frequent inferential error is comparing within-stratum p-values to claim effect modification. An estimate with p=0.02 below age 70 and p=0.15 above age 70 does not show that the effects differ. Estimate the between-stratum contrast on a prespecified scale—often through an interaction term, standardized contrasts, or another model-appropriate procedure—and report its interval. Different p-values can reflect different precision rather than different effects.
