...
Common PICO failures involve unspecified comparators and composites. 'Is EVT beneficial?' is incomplete without the comparator, population, outcome, and care era. A composite such as stroke, myocardial infarction, or vascular death can be driven by a frequent component of different importance from the others. If a synthetic composite result is driven by asymptomatic biomarker events while stroke estimates remain compatible with no benefit, disaggregate the components and uncertainty. Identifying the mismatch defines the next appraisal question; it does not establish intent or complete the appraisal by itself.
...
Estimand mismatch is a primary diagnostic finding in critical appraisal. When an abstract implies a broad treatment-policy benefit but the highlighted result targets a per-protocol or hypothetical estimand, the claim outruns the estimate unless both are clearly distinguished. Evaluate the result according to the population, treatment conditions, intercurrent-event strategy, outcome, horizon, and summary measure actually analyzed; compare those elements with the abstract and discussion without inferring intent.
...
In observational stroke research, index time is sometimes left implicit in the dataset. Researchers might use admission, stroke onset, discharge, a pharmacy fill, or a clinic visit as t0. The choice can materially alter the target population, strategy definition, and interpretation. If an analysis compares medication receipt during a hospital stay but starts follow-up at admission, it must represent treatment timing explicitly; classifying future receipt as a baseline exposure can decouple assignment from eligibility and create immortal-time or selection bias.
...
Target-trial emulation makes index-time discipline explicit by requiring a protocol that aligns eligibility, strategy assignment, and follow-up or represents grace periods and time-varying strategies appropriately. Checking that alignment is a core appraisal step, especially before interpreting a highly adjusted association whose exposure definition uses future information.
...
Clinical decisions require absolute outcome probabilities as well as relative effects. An NNH of 100 for a defined hemorrhage and an NNT of 20 for a defined stroke outcome can aid communication, but the two reciprocal counts are not commensurate without horizons, uncertainty, outcome severity, competing events, and patient values. When appropriate, calculate crude risk differences from raw event counts as a transparency check; do not substitute that crude calculation for a prespecified adjusted, time-to-event, ordinal, or competing-risk estimand.
...
By definition, patients ultimately classified as attending rehabilitation survived until their first session. If this waiting time is credited to the exposed group or early deaths are assigned only to non-attenders, the estimated association will be biased in favor of attendance even if rehabilitation has no mortality effect. Remedies depend on the estimand: a day-90 landmark analysis answers a survivor-population question; a time-varying analysis or sequential target-trial emulation can address treatment timing under additional assumptions.
...
Your primary defense against these failure modes is to map the chronology. If a paper does not show when eligibility, strategy assignment, exposure, and follow-up begin—or how pre-exposure person-time is handled—its protective interpretation remains unsupported until those details are clarified.
...
Many clinical contrasts can be anchored to a decision time, while others involve dynamic or repeated decisions that require time-varying strategies. Acute decisions about thrombolysis, EVT, or antiplatelet initiation still need an explicit eligibility and assignment origin. Papers that do not align these elements—or model their timing appropriately—may estimate a contrast different from the bedside question. Index-time clarity is therefore a clinical safety practice, not mere notation.
...
Estimand thinking clarifies shared decision-making. A family asking about outcomes under thrombectomy versus its alternative usually needs a treatment-policy contrast that includes downstream consequences such as procedural hemorrhage, access-site complications, and unsuccessful reperfusion. Analysis by randomized assignment can preserve that contrast, but missing outcomes and measurement still require explicit handling. A hypothetical or while-on-treatment estimand may answer a different mechanistic question and should not be substituted without explanation. Match the population, strategies, outcome, horizon, and intercurrent-event strategy to the conversation.

For researchers using electronic health records and large observational datasets, writing a target-trial protocol before analysis—defining eligibility, index time, treatment strategies, outcomes, and causal contrasts—can prevent common immortal-time errors and expose ill-posed PECO questions. It does not guarantee unbiased estimates, but it gives reviewers a clear estimand and protocol to interrogate.
